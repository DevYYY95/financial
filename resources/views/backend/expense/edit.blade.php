        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('expense.update',$data->id) }}" autocomplete="off"
                            enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            <h6 class="heading-small text-muted mb-4">{{ __('Edit expense') }}</h6>
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="expense_cat_id">
                                        ...
                                        @foreach($category as $row)
                                            <option value="{{$row->id}}" {{$data->expense_cat_id == $row->id ? 'selected' : ''}}>{{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
